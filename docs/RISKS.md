...
| R-12 | DataHub summary storage is mistaken for transactional campaign state | high | contained | SQLite events rebuild the manifest across restart and interrupted materialization; no DataHub state participates | Phase 02/04 write failure must leave this local state resumable |
