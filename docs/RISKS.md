...
| R-06 | Partial native mutation leaves a consumer in a worse state | critical | testing | One-file Git apply is atomic. Looker writes durable intent before POST/PATCH, never retries a mutation automatically, rereads an unknown outcome, verifies actual target and changed fields, and preserves an intervening edit during compensation | Phase 07 must inject more write-boundary faults; live Looker loss/cancellation and compensation remain |
| R-07 | Overlapping campaigns mutate the same consumer inconsistently | high | contained for campaign kernel | Both native workflows acquire the same unique identity claim only after exact approval and before mutation; the Looker workflow uses project, model, Explore, folder, object, content-metadata, and creation identity | Live concurrent Looker attempts remain unobserved |
...
| R-14 | Apply credentials are too broad or leak through evidence | critical | testing | Apply is opt-in and target-bound. Looker config redacts credentials, live mode requires verified TLS, the no-secret packet reports names/status only, recipes use runtime secret references, and injected recognizable secrets did not reach disk or stdout | Effective least-privilege permissions and deployed secret storage still require live Looker evidence |
...
| R-18 | Native retries duplicate or broaden mutations | high | testing | Repeated Git/dbt apply retained one commit. Deterministic Looker apply uses immutable-query reuse, one-property target equality, no automatic mutation retry, and reread recovery that leaves PATCH count at one after a lost response | Live Looker idempotency and service-side duplicate-query behavior remain |
...
| R-31 | A timeout or cancellation after apply leaves mutation outcome unknown and a retry duplicates or widens the change | critical | testing | Looker intent is durable as `PENDING`, `OUTCOME_UNKNOWN`, or `RESOLVED`; dropped-response and hard-interruption fixtures recover only by native reread and prove one PATCH, while unmatched state remains unknown | Repeat timeout, cancellation, and lost-response recovery against the live API |
