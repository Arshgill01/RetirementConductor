...
| R-02 | A DataHub entity cannot be mapped reliably to the exact native object | critical | testing | Live exact dataset and field-pair resolution rejects component guesses; display names, ownership, and table edges have no mutation authority | Phase 03 and 06 must bind each mutable consumer to one native Git/dbt or Looker identity |
...
| R-05 | A source changes between plan and apply | critical | testing | Product fingerprint refusal now passes in fixture mode; prior dbt experiment remains the only live observation | Phase 03 reproduces stale-source refusal without mutation against the disposable Git/dbt target |
...
| R-07 | Overlapping campaigns mutate the same consumer inconsistently | high | contained | One SQLite writer owns a unique active native-identity claim; the two-campaign integration case refuses the second apply | Live adapters must use the same claim before native mutation |
...
| R-14 | Apply credentials are too broad or leak through evidence | critical | testing | Blank secret references, repository scan, and public-artifact scan pass; apply authorization is not implemented yet | Negative tests prove plan-only principals cannot apply and live adapters demonstrate least privilege |
...
| R-18 | Native retries duplicate or broaden mutations | high | testing | Kernel event retries are idempotent across every commit boundary and conflicting key reuse refuses | Native phases must reconcile outcome-unknown retries and exact actual targets |
...
| R-24 | Generated changes are overbroad, templated, or miss indirect references | high | testing | Phase 00 traversal and allowlist fixtures refuse before mutation; no generated patch exists yet | Aliases, macros, generated SQL, and unauthorized-file fixtures behave safely in phase 03 |
