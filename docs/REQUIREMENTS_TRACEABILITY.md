...
| RC-022 | Isolate the safety value of revocable permission from a static retirement sign-off under the same late-consumer intervention | `docs/DECISIONS.md` | post-goal | [Lease value comparison](../artifacts/public/lease-value-comparison-v1/index.json) retained stale green in the frozen executable static arm while the inspected live-local lease became `INVALIDATED`, refused the old gate plan, and produced zero actions |
| RC-023 | Let a bounded Superset native receipt participate consequentially in canonical campaign readiness beside Git/dbt | `docs/CONTRACTS.md` | post-goal | [Heterogeneous campaign evidence](../artifacts/public/heterogeneous-campaign/index.json) binds two native migrations and receipts; the live-local campaign changed `READY_TO_RETIRE` to `UNSAFE` when Superset was compensated. Gate-time Superset refresh remains explicitly outside acceptance |
