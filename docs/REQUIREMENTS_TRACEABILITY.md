...
| RC-016 | Fail closed under least privilege, tampering, concurrency, partial failure, retries, untrusted input, and recovery | `docs/RISKS.md` | 07 | [Post-removal Phase 07 acceptance](EVIDENCE_LEDGER.md#phase-07-post-removal-acceptance): plan/apply separation, fault matrix, manifest-preserving recovery, copied-store refusal, benchmark binding, dependency audit, and scans passed |
| RC-017 | Install, configure, upgrade, restore, and remove the product reproducibly from a clean environment | `docs/PRODUCT.md` | 08 | [Post-removal Phase 08 engineering acceptance](EVIDENCE_LEDGER.md#phase-08-post-removal-engineering-acceptance): reproducible package, four clean runtimes, actionable preflight, installed live Core reference, upgrade/rollback, backup, copied-state refusal, confirmed removal, and uninstall |
| RC-018 | Establish whether another real operator can use the workflow and obtains recurring value | `docs/PRODUCT.md` | follow-on | [Phase 08 operator boundary](../artifacts/public/phase08/operator-boundary.json) is `NOT_RUN`; independent run and redacted frequency, friction, value, willingness, and buyer observations remain required and no adoption claim is made |
| RC-019 | Prove evidence quality and zero false readiness against pinned official datasets plus an independent deterministic truth oracle | `GOAL.md` | 06 | [EP-006](EVIDENCE_LEDGER.md#phase-06-post-removal-confirmation): four pinned official assets, deterministic corpus and independent oracle, direct live-local DataHub readback, exact Git/dbt receipt, 14/14 scenario matches, and zero false readiness after removal |
| RC-020 | Observe an issued Retirement Lease from fresh equivalent evidence, publish the result, and invalidate the lease on any changed canonical state | `docs/CONTRACTS.md` | post-goal | [Continuous Retirement Lease evidence](EVIDENCE_LEDGER.md#post-goal-continuous-retirement-lease-evidence): live exact field consumer reversed readiness, publication read-back passed, the preserved lease became `INVALIDATED`, and the exact old plan refused without another sentinel |
| RC-021 | Select the supported agent and planning architecture from predeclared comparative evidence without promoting behavioral non-use into capability containment | `docs/DECISIONS.md` | post-goal | [Truth-experiment integration evidence](EVIDENCE_LEDGER.md#post-goal-truth-experiment-integration-evidence) selected removal of nested Gemini and retention of bounded DataHub, the skill, and product MCP; [TE-04 definitive evidence](EVIDENCE_LEDGER.md#te-04-definitive-unified-agent-evidence) exercised that architecture with exact public PR/CI, publication, lease reversal, zero producer actions, and an explicit non-capability-bounded host inventory |
| RC-022 | Compare revocable permission fairly against both static sign-off and competent fresh action-time CI under the same late-consumer intervention | `docs/DECISIONS.md` | post-goal | The earlier [lease value comparison](../artifacts/public/lease-value-comparison-v1/index.json) isolated stale static authority; the [definitive consequential comparison](../artifacts/public/definitive-consequential-run/comparison.json) then found competent fresh CI safety-equivalent with less persistent machinery. Classification: `NO_MATERIAL_ADVANTAGE`; recommendation: `SIMPLIFY` |
| RC-023 | Let a bounded Superset native receipt participate in readiness and require independent final gate-time native verification | `docs/CONTRACTS.md` | post-goal | [Definitive clean and negative evidence](../artifacts/public/definitive-consequential-run/index.json) binds least-privilege database/dataset/chart reread plus forced saved-chart execution; native Superset drift refused before PostgreSQL mutation. Production Superset support remains unclaimed |
| RC-024 | Execute one exact PostgreSQL producer action behind durable one-use intent and resolve ambiguous outcomes without blind retry | `docs/CONTRACTS.md` | post-goal | [Definitive consequential evidence](../artifacts/public/definitive-consequential-run/index.json) committed one clean drop, refused replay and all unsafe cases, recorded crash-before-client as `NOT_COMMITTED`, retained lost response as `OUTCOME_UNKNOWN`, and resolved it through native reread with one execute call |
| RC-025 | Make fresh verification and the explicitly selected producer action one trusted default invocation without operator-supplied lease timestamps | `docs/CONTRACTS.md` | post-goal | Focused gate and CLI tests prove runtime-owned timing, prepare-then-execute ordering, explicit action selection, and the retained 15-minute maximum internal lifetime; the complete repository acceptance is recorded in the post-CP-05 simplification evidence ledger entry |
