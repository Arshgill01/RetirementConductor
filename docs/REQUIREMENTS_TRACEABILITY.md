...
| RC-008 | Bind every mutable consumer one-to-one to an exact native identity | `docs/CONTRACTS.md` | 03, 06 | [EP-003](EVIDENCE_LEDGER.md#ep-003--phase-03-git-and-dbt-execution): one exact fresh DataHub URN to dbt manifest identity; Looker and recycled-identity evidence remain |
...
| RC-016 | Fail closed under least privilege, tampering, concurrency, partial failure, retries, untrusted input, and recovery | `docs/RISKS.md` | 07 | [Phase 07 credential-independent observations](EVIDENCE_LEDGER.md#phase-07-credential-independent-observations): plan-only receipts, fault matrix, recovery drill, copied-store refusal, and scans pass; live Looker-specific acceptance remains |
